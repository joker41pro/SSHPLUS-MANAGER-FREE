  ...
  if [[ "$respo" = '1' ]]; then
    ...
    sleep 1.5
      ...
      fun_packhost () {
        ...
        cp "$HOME/$username.ovpn" "$HOME/OVPN/$username-bypass.ovpn"
        cd "$HOME/OVPN" && zip "$username.zip" *.ovpn > /dev/null 2>&1 && cp "$username.zip" "$HOME/$username.zip"
        cd "$HOME" && rm -rf "$HOME/OVPN" > /dev/null 2>&1
      }
